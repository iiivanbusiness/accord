...
export async function createReviewChain(formData: FormData) {
...
export async function deleteReviewChain(chainId: string) {
  const user = await requirePermission("canManageWorkspace");
...
export async function moveReviewChain(chainId: string, direction: "up" | "down") {
  const user = await requirePermission("canManageWorkspace");
  const chain = await prisma.reviewChain.findFirst({ where: { id: chainId, workspaceId: user.workspaceId } });
  if (!chain) throw new Error("Chain not found");
...
export async function addReviewChainStep(chainId: string, formData: FormData) {
  const user = await requirePermission("canManageWorkspace");
  const assigneeId = String(formData.get("assigneeId") ?? "");
  if (!assigneeId) throw new Error("Choose a teammate");
  ...
  if (existing) throw new Error("That person is already a step in this chain");
...
export async function removeReviewChainStep(stepId: string) {
  const user = await requirePermission("canManageWorkspace");
...
export async function moveReviewChainStep(stepId: string, direction: "up" | "down") {
  const user = await requirePermission("canManageWorkspace");
  const step = await prisma.reviewChainStep.findFirst({ where: { id: stepId, chain: { workspaceId: user.workspaceId } } });
  if (!step) throw new Error("Step not found");
