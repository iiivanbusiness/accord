...
export async function createLibraryClause(formData: FormData): Promise<{ error?: string }> {
  const title = String(formData.get("title") ?? "").trim();
  const body = String(formData.get("body") ?? "").trim();
  if (!title) return { error: "Title is required" };
  if (!body) return { error: "Clause text is required" };
...
export async function updateLibraryClause(itemId: string, formData: FormData): Promise<{ error?: string }> {
  const title = String(formData.get("title") ?? "").trim();
  const body = String(formData.get("body") ?? "").trim();
  if (!title) return { error: "Title is required" };
  if (!body) return { error: "Clause text is required" };
...
export async function deleteLibraryClause(itemId: string): Promise<{ error?: string }> {
  const user = await requirePermission("canApproveTemplates");
