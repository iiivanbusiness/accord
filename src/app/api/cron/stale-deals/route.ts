...
import { sendAdminAlertEmail, sendStaleDealsEmail } from "@/lib/email";

const STALE_AFTER_DAYS = 7;
const DAY_MS = 24 * 60 * 60 * 1000;

// Runs daily (see vercel.json). A deal sitting in "ready" or "missing_info"
// — someone needs to review it or fill something in, and nothing else
// nudges that — with no activity for a week gets bundled into one digest
// email per workspace, not one email per deal. Re-fires every 7 days for
// as long as it stays genuinely untouched (staleReminderSentAt only counts
// if it's *after* the deal's last real update), so a deal that's been
// forgotten for a month doesn't just quietly stop being mentioned after
// the first nudge.
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ checked: staleDeals.length, sent });
  } catch (err) {
